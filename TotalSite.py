...
variableExplicar = 'OGMV_SEARCH'

# %%

# Leemos el archivo
file = pd.read_csv('totalsite.csv')
file = file[file['SEARCH_VERTICAL'] == 'ROPA Y ACCESORIOS']

# %%
# %%
# Para este análisis no usamos la vertical (podemos incluirla como categorica)
file.drop(columns=['SEARCH_VERTICAL','DSA_VERTICAL','SITE_VERTICAL'], inplace=True)
...
file['average'] = np.array(file.set_index(['TIM_DAY']).sort_index(ascending=False)[
                           variableExplicar].rolling(14).mean().sort_index(ascending=True).copy())
...
labels = np.array(features[variableExplicar])
# Tenemos que sacar costo de los features
# usamos axis = 1 para dropear a nivel columna
features = features.drop(variableExplicar, axis=1)
